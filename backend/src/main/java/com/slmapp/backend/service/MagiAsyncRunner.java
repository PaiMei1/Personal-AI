...
// Pulled into its own bean because @Async only takes effect when the method is invoked
// through the Spring proxy - calling it from inside the same class (self-invocation)
// silently runs it synchronously instead, which is what was happening before this split.
@Service
public class MagiAsyncRunner {
    ...
    public MagiAsyncRunner(SlmClient slmClient,
